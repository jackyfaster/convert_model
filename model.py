...
momentum_rate = 0.9
kernel_size = 3
log_save_path = './vgg_19_logs/my_logs'
model_save_path = './model/bn/vgg19.ckpt'
...
def load_data_one(file, code=True):
    batch = unpickle(file, code)
    data = batch[b'data']
...
def conv2d(input, out_channels):
    return tf.layers.conv2d(inputs=input, filters=out_channels,  kernel_size=kernel_size, padding='SAME')


def max_pool(input, k_size=1, stride=1, name=None):
    return tf.layers.max_pooling2d(input, pool_size=k_size, strides=stride,
                          padding='SAME', name=name)


def batch_norm(input):
    return tf.layers.batch_normalization(inputs=input, training=train_flag)
    # return layers.batch_norm(input, decay=0.9, center=True, scale=True, epsilon=1e-3,
...
if __name__ == '__main__':
    ...
    output = tf.nn.relu(batch_norm(conv2d(x, out_channel1)))

    out_channel2 = 64
    # b_conv1_2 = bias_variable([64])
    output = tf.nn.relu(batch_norm(conv2d(output, out_channel2)))
    output = max_pool(output, 2, 2, "pool1")

    out_channel3 = 128
    # b_conv2_1 = bias_variable([128])
    output = tf.nn.relu(batch_norm(conv2d(output, out_channel3)))

    out_channel4 = 128
    # b_conv2_2 = bias_variable([128])
    output = tf.nn.relu(batch_norm(conv2d(output, out_channel4)))
    output = max_pool(output, 2, 2, "pool2")

    out_channel5 = 256
    # b_conv3_1 = bias_variable([256])
    output = tf.nn.relu(batch_norm(conv2d(output, out_channel5)))

    out_channel6 = 256
    # b_conv3_2 = bias_variable([256])
    output = tf.nn.relu(batch_norm(conv2d(output, out_channel6)))

    out_channel7 = 256
    # b_conv3_3 = bias_variable([256])
    output = tf.nn.relu(batch_norm(conv2d(output, out_channel7)))

    out_channel8 = 256
    # b_conv3_4 = bias_variable([256])
    output = tf.nn.relu(batch_norm(conv2d(output, out_channel8)))
    output = max_pool(output, 2, 2, "pool3")

    out_channel9 = 512
    # b_conv4_1 = bias_variable([512])
    output = tf.nn.relu(batch_norm(conv2d(output, out_channel9)))

    out_channel10 = 512
    # b_conv4_2 = bias_variable([512])
    output = tf.nn.relu(batch_norm(conv2d(output, out_channel10)))

    out_channel11 = 512
    # b_conv4_3 = bias_variable([512])
    output = tf.nn.relu(batch_norm(conv2d(output, out_channel11)))
    ...
    with tf.Session() as sess:
        ...
        for ep in range(1, total_epoch+1):
            ...
            for it in range(1, iterations+1):
                ...
                batch_acc = accuracy.eval(feed_dict={x: batch_x, y_: batch_y, keep_prob: 1.0, train_flag: False})
